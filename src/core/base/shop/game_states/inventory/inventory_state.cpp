...
ExolorGame::InventoryState::InventoryState(Game *game) : ExolorGame::GameMenu(game, "Inventory Menu"),
                                                         inventory(new ExolorGame::Inventory(18)) {
...
void displayErrorMessage(GameRenderer::TextCanvas *canvas, const std::string &message) {
    canvas->drawSquare(Vec2((canvas->getWidth() - message.length()) / 2, 10), message.length() + 2, 5, '*', message,
                       true);
}
...
void ExolorGame::InventoryState::initCommands() {
    ...
    addCommand("sell", [canvas](Game *g, InputParameter &param) {
        ...
        if (g->getCharacter() == nullptr) {
            ...
            canvas->drawSquare(Vec2((canvas->getWidth() - errorMessage.length()) / 2, 10), errorMessage.length() + 2, 5,
                               '*', errorMessage, true);
            return;
        }
        ...
        if (countStr == "all") {
            ...
            try {
                ...
                canvas->drawSquare(Vec2((canvas->getWidth() - errorMessage.length()) / 2, 10),
                                   errorMessage.length() + 2, 5, '*', errorMessage, true);
                return;
            }
    ...
    addCommand("show", [canvas](Game *g, InputParameter &param) {
        ...
        std::cout << "Rendering inventory at position (" << centerPosition.x << ", " << centerPosition.y << ")"
                  << std::endl;
    ...
    addCommand("select", [this, canvas](Game *g, const InputParameter &param) {
        ...
        canvas->drawText(Vec2(squareTopLeft.x + 2, squareTopLeft.y + 11),
                         "Price per Item: " + std::to_string(itemPrice));
